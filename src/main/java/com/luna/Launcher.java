...
import com.luna.data.DownloadableFile;
import com.luna.data.Instance;
import com.luna.data.LauncherVersion;
import com.luna.graphql.AddLauncherLaunchMutation;
import com.luna.graphql.type.AddLauncherLaunchInput;
import com.luna.graphql.type.LauncherJavaVersionInput;
import com.luna.gui.dialogs.ProgressDialog;
import com.luna.gui.tabs.PacksBrowserTab;
...
public class Launcher {
    ...
    public void loadEverything() {
        ...
        if (OS.isWindows() && !Java.is64Bit() && OS.is64Bit()) {
            ...
            if (ret == 0) {
                OS.openWebBrowser("https://luna.com/help/32bit/");
                System.exit(0);
            }
        }
     ...
     * needs to have
     */
    private List<com.luna.network.Download> getLauncherFiles() {
        if (this.launcherFiles == null) {
            java.lang.reflect.Type type = new TypeToken<List<DownloadableFile>>() {
            }.getType();

            try {
                this.launcherFiles = com.luna.network.Download.build().cached()
    ...
    public boolean hasUpdatedFiles() {
        ...
        List<com.luna.network.Download> downloads = getLauncherFiles();

        if (downloads == null) {
            return false;
        }

        return downloads.stream().anyMatch(com.luna.network.Download::needToDownload);
    }
            ...
            if (App.noLauncherUpdate) {
                int ret = DialogManager.okDialog().setTitle("Launcher Update Available")
                        .setContent(new HTMLBuilder().center().split(80).text(GetText.tr(
                                "An update to the launcher is available. Please update via your package manager or manually by visiting https://luna.com/downloads to get the latest features and bug fixes."))
                                .build())
                        .addOption(GetText.tr("Visit Downloads Page")).setType(DialogManager.INFO).show();

                if (ret == 1) {
                    OS.openWebBrowser("https://luna.com/downloads");
                }
